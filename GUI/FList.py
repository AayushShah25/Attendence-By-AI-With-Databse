    ...
    def __init__(self,Data):
        ...
        def imprv(id):
            ...
            if str(id) in faceList:
                path = r'C:\Users\GIGABYTE\Desktop\Project\Faces\\'+str(2)
                
                total = os.listdir(path)
                paths = [os.path.join(r'C:\Users\GIGABYTE\Desktop\Project\Faces\\'+str(2), basename) for basename in total]
                latest = max(paths, key=os.path.getctime)
                _, t = os.path.split(latest)
                name = t.split('.')
                name = name[0]
                top.destroy()
                CaptureScreen.Capture(id,int(name))
